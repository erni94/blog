  ...
  if (!isLoggedIn) {
    return (
      <header className={'header'}>
        <div>
          <Link className={'title'} to={'/'}>Realworld Blog</Link>
        </div>
        <Button onClick={() => navigate('/sign-in')} type='text' className={'signin-button'} size={'large'}>
          Sign In
        </Button>
        <Button onClick={() => navigate('/sign-up')} className={'signup-button'} size={'large'}>Sign Up</Button>
      </header>
    );
  } else {
    return (
      <header className={'header'}>
        ...
        <div className={'header__username'} onClick={() => navigate('/profile')}>{username}</div>
        <Avatar size={46} src={image} className={'header__avatar'} onClick={() => navigate('/profile')} />
        <Button onClick={() => dispatch(logOut())} className={'logout-button'} size={'large'}>Log Out</Button>
      </header>
    );
  }
